...
class WavLMPositionalConvEmbedding(nn.Module):
    ...
    def forward(self, hidden_states, chunk_size=-1):
        hidden_states = hidden_states.transpose(1, 2)

        if chunk_size < 0 or chunk_size >= self.right_context:
            hidden_states = self.conv(hidden_states)
        else:
...
class WavLMAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_bias is not None:
            if attention_bias.shape != (bsz, tgt_len, tgt_len):
                raise ValueError(f"Expected shape of attention bias is {(bsz, tgt_len, tgt_len)}, "
                                  "but got {attention_bias.shape}")
            attention_bias = attention_bias.unsqueeze(1).repeat(1, self.num_heads, 1, 1).view(-1, tgt_len, tgt_len)
...
class WavLMEncoder(nn.Module):
    ...
    def forward(
        ...
        position_embeddings = self.pos_conv_embed(hidden_states, chunk_size=chunk_size)
        ...
        for i, layer in enumerate(self.layers):
            ...
            layer_attention_bias = attention_bias
